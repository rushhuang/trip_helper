...
- [ ] **行程選擇器列**（list-view 頁面頂部，日期 tab 上方）
  - 僅有 ≥ 2 個行程時顯示，單行程隱藏
  - 以橫向捲動 pill 呈現所有行程標題
  - 選中行程以高亮 pill 標示，切換時更新日期 tab 及內容
  - 不需進入管理頁即可完成切換
- [ ] **管理頁行程列表同步高亮**（行程選擇器切換後，管理頁 active badge 同步）
- [ ] **地圖頁行程 badge**（地圖右上角小 pill 顯示當前行程名稱，點擊跳回列表頁）
...
- [ ] **響應式斷點**：`@media (min-width: 768px)` 啟用電腦版佈局
- [ ] **頂部導覽列**（電腦版取代底部導覽）
  - 左：App 標題
  - 中：行程選擇器 pill（Phase 9）
  - 右：搜尋框（常駐展開）、列表／地圖切換按鈕
- [ ] **左側邊欄**：日期 tab 改為垂直列表，固定高度可捲動
- [ ] **主內容區**：列表或地圖佔滿剩餘寬度，地圖高度 100vh
- [ ] **卡片寬度上限**：站點卡片 max-width，避免超寬螢幕過度拉伸
- [ ] **行程管理**：電腦版改為右側 panel 或 modal，不佔整頁
