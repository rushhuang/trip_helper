...
- [x] **deploy.sh**：互動式工具選單，分轉換／部署兩大區塊
  ...
  - 部署（遠端）
    - `7` GitHub Pages（`git subtree split` + force push `gh-pages` 分支 + 自動設定 Pages）
    - `8` Netlify（`git archive` 建立乾淨部署目錄 + `netlify deploy --prod`）
    - `9` GitHub Release（`git archive` 打包 ZIP + `gh release create`，含自動版號建議）
