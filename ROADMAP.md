...
- [x] **deploy.sh**：互動式部署選單
  - 本地 HTTP `localhost:8080`（快速測試）
  - 本地 HTTPS `localhost:8443`（PWA 完整功能）
  - HTTPS + adb 轉發（Android 手機安裝，含裝置偵測）
  - 遠端部署預留：GitHub Pages / Netlify / GitHub Release（TODO）
...
# 部署到 GitHub Pages（待 deploy.sh 選項 4 實作）
