...
- [x] **deploy.sh**：互動式工具選單（單字元選項 `1-9, a-c, q`）
  - 轉換
    - `1` xlsx → JSON（自動偵測根目錄 .xlsx，缺套件自動安裝）
    - `2` JSON → xlsx（行程資料轉 Excel）
    - `3` 地理編碼（Nominatim，缺套件自動安裝，先檢查 JSON 存在）
    - `4` MapCode 查詢（japanmapcode.com，需有座標）
    - `5` 完整流程（依序執行 1 → 3 → 4）
    - `6` 單元測試（執行 Python 測試）
  - 部署（本地）
    - `7` HTTP `localhost:8080`（快速測試）
    - `8` HTTPS `localhost:8443`（PWA 完整功能，自動找可用 port）
    - `9` HTTPS + adb 轉發（Android 手機安裝，含裝置偵測）
  - 部署（遠端）
    - `a` GitHub Pages（`git subtree split` + force push `gh-pages` 分支 + 自動設定 Pages）
    - `b` Netlify（`git archive` 建立乾淨部署目錄 + `netlify deploy --prod`）
    - `c` GitHub Release（`git archive` 打包 ZIP + `gh release create`，含自動版號建議）
...
- [x] **營業時間衝突警示**：比對站點到達時間與 `hours` 欄位，以下情況顯示黃色警示
  - 到達時間早於開始營業（如 09:00 開門但 08:30 到達）
  - 到達時間晚於結束營業（如 17:00 關門但 17:30 到達）
  - 落在中午休息時段（如 `11:00-14:00, 17:00-21:00` 的 14:00–17:00 空檔）
  - 公休日（如 `hours` 含「水曜定休」「週X公休」而行程日為該星期）
  - 全天開放 / 全年無休自動跳過
  - 警示顯示於站點卡片上，提示具體原因（如「⚠️ 尚未營業」「⚠️ 已打烊」「⚠️ 休息時段」「⚠️ 公休日」）
...
- [x] **匯入格式選擇**：行程管理頁「匯入」同時支援 `.json` 和 `.xlsx`
- [x] **瀏覽器端解析 xlsx**：使用 [SheetJS (xlsx)](https://docs.sheetjs.com/) 讀取 Excel
- [x] **欄位對應**：與 `xlsx_to_json.py` 相同邏輯（`pwa/xlsx-utils.js`）
  - A 欄：日期（`3/19(四)` 格式，往下延伸至 None）
  - B 欄：時間
  - C 欄：地點名稱
  - D 欄：五段式資訊（Mapcode / 地址 / 電話 / 營業時間 / 🅿️）
  - E 欄：備注
- [x] **停留/交通時間解析**：從 D/E 欄提取 `duration` 和 `travelTime`（同 Phase 14 邏輯）
- [x] **匯入提示**：匯入後 toast 提示不含自動 geocode / MapCode

#### 15-B PWA 匯出支援 xlsx

- [x] **匯出格式選擇**：匯出按鈕彈出格式選擇器（JSON / Excel）
- [x] **瀏覽器端產生 xlsx**：使用 SheetJS 產生 `.xlsx` 下載
- [x] **欄位還原**：還原為原始 A–E 欄格式
- [x] **欄寬自動調整**（SheetJS 社群版不支援儲存格底色）

#### 15-C 轉換腳本 `scripts/json_to_xlsx.py`（保留 Python 流程）

- [x] **讀取 itinerary.json**（或指定路徑）產生 `.xlsx`
- [x] **欄位對應與樣式**：A–E 欄還原 + 每日標題列底色 + 粗體 + 欄寬
- [x] **支援 `--json` 和 `--output` 參數**

#### 15-D deploy.sh 整合

- [x] 工具選單新增「JSON → xlsx」選項（選項 2）

#### 15-E 單元測試

- [x] `tests/test_xlsx_to_json.py`：類型偵測、D 欄解析、停留/交通時間、時間格式化（33 項）
- [x] `tests/test_json_to_xlsx.py`：欄位對應、樣式、底色、邊界情況（22 項）
- [x] `tests/test_roundtrip.py`：JSON → xlsx → 解析 資料保真度（11 項）
- [x] `tests/test-xlsx-utils.html`：瀏覽器端 JS parseXlsx / exportXlsx 往返測試
...
**目標：** 在地圖頁新增「我的位置」按鈕，使用瀏覽器 Geolocation API 顯示使用者當前位置。

### 功能清單

- [x] **「📍」按鈕**：地圖控制列新增第三個按鈕（全部 | 今日 | 📍）
- [x] **GPS 定位**：使用 `navigator.geolocation.getCurrentPosition()` 取得座標
- [x] **藍色圓點標記**：Google Maps 風格藍色圓點 + 白色邊框 + 陰影
- [x] **精確度範圍圈**：以半透明藍色圓圈顯示 GPS 精確度範圍
- [x] **定位中動畫**：按鈕閃爍提示正在取得位置
- [x] **錯誤處理**：權限拒絕、無法定位、逾時等情況顯示 toast 提示
- [x] **按鈕重新命名**：原「定位」→「全部」，避免與 GPS 定位混淆
